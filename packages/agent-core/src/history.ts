...
export function countAsked(messages: WireMessage[]): number {
  return messages.filter(
    (m) =>
      m.role === 'assistant' && (m.tool_calls ?? []).some((tc) => tc.function?.name === 'ask_user_question'),
  ).length;
}
...
export async function appendAnswer(
  ...
  const unanswered = (): RawToolCall[] =>
    (lastAssistant.tool_calls ?? []).filter(
      (tc) => !messages.some((m) => m.role === 'tool' && m.tool_call_id === tc.id),
    );
  ...
  for (const tc of unanswered()) {
    if (tc.function?.name === 'ask_user_question') {
      messages.push({
        role: 'tool',
        tool_call_id: tc.id,
        content: JSON.stringify({ skipped: '一次只回答一个问题' }),
      });
