 ...
 *
 * 与外部数据的边界：客户端请求体见 src/wire/messages.ts，
 * 与 AI SDK ModelMessage 的互转见 src/wire/model-messages.ts；
 * 业务代码只消费本文件的定义。
 */
...
export interface WireMessage {
   ...
   * 由 AI SDK 解析响应得到，随历史保留并回传给客户端；
   * 发送给 LLM 时会被剥离（见 wire/model-messages.ts 的「reasoning 只进不出」）。
   */
...
/** POST /api/agent（及云函数 gift-agent）的统一响应 */
